    ...
    combinators: {
        ' ': function(node, token) {
            while ((node = node[PARENT_NODE])) {
                if (Selector._testNode(node, '', token.prev)) {
                    return true;
                }
            }  
            return false;
        ...
        '>': function(node, token) {
            return Selector._testNode(node[PARENT_NODE], null, token.prev);
        },
        '+': function(node, token) {
            ...
            if (sib && Selector._testNode(sib, null, token.prev)) {
                return true; 
            }
            return false;
        },
        ...
        '~': function(node, token) {
            var sib = node[PREVIOUS_SIBLING];
            while (sib) {
                if (sib[NODE_TYPE] === 1 && Selector._testNode(sib, null, token.prev)) {
                    return true;
                }
                sib = sib[PREVIOUS_SIBLING];
            }
